...
ADD CONSTRAINT FK_PDKT_DMVC FOREIGN KEY (MADON) REFERENCES DONMUAVC (MADON)
...
CREATE TABLE CT_PHIEUVC
(
	MAPDKTIEM	CHAR(8),
	MAVC		CHAR(8),
	NGAYTIEM	DATE,
	PRIMARY KEY	(MAPDKTIEM, MAVC)
)

ALTER TABLE CT_PHIEUVC 
ADD CONSTRAINT FK_VC_CT_PHIEUTIEM FOREIGN KEY (MAVC) REFERENCES VACCINE (MAVC)

ALTER TABLE CT_PHIEUVC 
ADD CONSTRAINT FK_PDK_CT_PHIEUTIEM FOREIGN KEY (MAPDKTIEM) REFERENCES PHIEUDKTIEM (MAPDKTIEM)
...
CREATE OR ALTER PROC USP_Vaccine_Add
	@MAVC		CHAR(8),
	@SOLUONG	INT
AS
BEGIN
	UPDATE VACCINE SET SLT = SLT + @SOLUONG WHERE MAVC = @MAVC
END;
...
CREATE OR ALTER PROC USP_Add_CT_DMVC
	@MADON		CHAR(8),
	@MAVC		CHAR(8),
	@SOLUONG	INT,
	@GIA		DECIMAL (19,4)
AS
BEGIN
	IF EXISTS (SELECT MADON, MAVC 
				FROM CT_DMVC
				WHERE MADON = @MADON AND MAVC = @MAVC)
	BEGIN
		UPDATE CT_DMVC
	...
	ELSE
	BEGIN
		INSERT INTO CT_DMVC(MADON, MAVC, SOLUONG, GIA) VALUES(@MADON, @MAVC, @SOLUONG, @GIA)
	END

	UPDATE DONMUAVC SET TONGTIEN = (SELECT SUM(GIA) FROM CT_DMVC WHERE MADON = @MADON) 
	WHERE MADON = @MADON;
END;
